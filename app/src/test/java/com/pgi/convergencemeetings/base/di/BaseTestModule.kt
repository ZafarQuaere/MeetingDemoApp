...
val baseTestModule = module {
	val appauth = mockkClass(AppAuthUtils::class, relaxed = true)
	val clientInfo = mockkClass(ClientInfoDaoUtils::class, relaxed = true)
	val loggerMock = mockkClass(Logger::class, relaxed = true)
	val logoutUserService = mockkClass(LogoutUserService::class, relaxed = true)
	val sharedViewModel = mockkClass(SharedViewModel::class, relaxed = true)
	...
	val featuresConfigService = mockkClass(FeaturesConfigService::class, relaxed = true)
	...
	every {
		logoutUserService.logoutUser(any())
	} answers { Observable.create<String> { "test" } }

	every {
		logoutUserService.revokeTokenLogoutUser(any())
	} answers { Observable.create<String> { "test" } }
...
val homeTestModuleWithNoMsalUser = module {
	...
	val sharedViewModelMock = mockkClass(SharedViewModel::class, relaxed = true)
	...
	factory { ProfileManager(gmElacticSearchRepoMock, loggerMock) }
	...
	viewModel { HomeCardsViewModel(graphAuthRepository, graphRepositoryMock, sharedViewModelMock,
			sharedPref, get(), loggerMock) }
}

val meetingsTestModuleWithNoMsalUser = module {
	val graphRepositoryMock = mockkClass(MSALGraphRepositoryImpl::class, relaxed = true)
	...
	val gmWebServiceRepositoryMock = mockkClass(GMWebServiceRepository::class, relaxed = true)
	...
	viewModel {
		MeetingsAgendaViewModel(graphAuthRepository, graphRepositoryMock, SharedViewModel(ApplicationProvider.getApplicationContext()),
				sharedPref, get(), loggerMock, gmWebServiceRepositoryMock)
	}
}