  ...
  module Actions
    ...
    class AnalyzeCommitsAction < Action
      ...
      def self.is_releasable(params)
        ...
        # Uncomment below code once we all reach a common agrrement on how to add commit messages.
        ...
        if params[:releaseVersion] == "major"
          next_major += 1
          next_minor = 0
          next_patch = 0
        elsif params[:releaseVersion] == "minor"
          next_minor += 1
          next_patch = 0
        elsif params[:releaseVersion] == "patch"
          next_patch += 1
        elsif params[:releaseVersion] == "default"
          next_minor += 1
        end

        if(params[:releaseVersion] == "default" )
          next_version = "#{next_major}.#{next_minor}.#{next_patch}-SNAPSHOT"
        else
          next_version = "#{next_major}.#{next_minor}.#{next_patch}"
        end
      ...
      def self.available_options
        ...
        [
          ...
          FastlaneCore::ConfigItem.new(
            key: :releaseVersion,
            description: "The version numberto bump",
