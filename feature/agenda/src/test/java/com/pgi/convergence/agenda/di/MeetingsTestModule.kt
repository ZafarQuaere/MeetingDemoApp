package com.pgi.convergence.agenda.di
...
val meetingsTestModuleWithNoMsalUser = module {
    ...
    val gmWebServiceRespositoryMock = mockkClass(GMWebServiceRepository::class, relaxed = true)

    var searchResult = SearchResult()
    var searchList = emptyList<SearchResult>()
    searchResult.hubConfId = 101202
    searchResult.furl = "pgi.globalmeet.com/test"
    ...
    coEvery {  sharedPref.firstName } returns "Test"
    coEvery {  sharedPref.lastName } returns "User"
    coEvery {  sharedPref.joinUrl } returns "http://pgi.com/test"
    coEvery {  sharedPref.profileImage } returns "https://id-data.globalmeet.com/r/profile-images/100x100/1b3e9d5c-3baa-4738-ae62-eaf5ce6bdcf3"

    val model = MeetingsAgendaViewModel(graphAuthRepository, graphRepositoryMock, SharedViewModel(ApplicationProvider.getApplicationContext()),  sharedPref, sharedFeatureViewModel, loggerMock, gmWebServiceRespositoryMock)
    viewModel { model }
}

@UnstableDefault
val meetingsTestModuleWithUserCount = module {
    val graphRepositoryMock = mockkClass(MSALGraphRepositoryImpl::class, relaxed = true)
    ...
    val calEvents = Json.parse(MsalCalEvent.serializer(), testMsalData)
    calEvents.value[0].start?.dateTime = DateTime.now().minus(10.minutes).format("yyyy-MM-dd'T'HH:mm:ss.SSSSSS")
    calEvents.value[0].end?.dateTime = DateTime.now().plus(1.hours).plus(10.minutes).format("yyyy-MM-dd'T'HH:mm:ss.SSSSSS")
    coEvery {
        graphRepositoryMock.getTimedEvents(any(), any(), any(), any())
    } returns calEvents
